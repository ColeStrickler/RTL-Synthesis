...
Verifier::Verifier(const std::vector<std::vector<int>> &input, const std::vector<int> &output, std::vector<InputNode *> input_nodes) : \
 m_Input(input), m_Output(output), m_InputNodes(input_nodes)

{
    ...
    BackTrackPermuteInputs(input_vec, input_nodes.size(), ret,  curr, count_map);


    input_combination_indexes = ret;
